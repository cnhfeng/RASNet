...
def get_loss_s(parser,seq_input,voc_size):
    diag = np.zeros((voc_size[0]))
    pro = np.zeros((voc_size[1]))
    med = np.zeros((voc_size[2]))
    diag[seq_input[-1][0]] = 1
    pro[seq_input[-1][1]] = 1
    med[seq_input[-1][2]] = 1
    ...
    # for _,d_target in enumerate(parser):
    #     null_diag = np.zeros((voc_size[0]))
    #     null_diag[seq_input[d_target][0]]=1
    #     loss += np.dot(diag, null_diag) / voc_size[0]
# 
    # for _,p_target in enumerate(parser):
